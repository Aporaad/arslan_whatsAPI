...
export class WwebjsMessaging {
  ...
  private async sendResolved<T>(
    ...
    try {
      ...
      if (to !== chatId && String((err as any)?.message || err).includes('Data passed to getter must include an id property')) {
        this.host.logger.warn(`Send to ${to} failed with memoize id error; retrying with original chatId ${chatId}`);
        try {
          return await send(chatId);
        } catch (retryOriginalErr) {
          this.host.logger.error(`Retry with ${chatId} also failed: ${retryOriginalErr}`);
        }
      }
