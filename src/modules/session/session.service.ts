...
export class SessionService implements OnModuleDestroy, OnModuleInit, OnApplicationBootstrap, PluginSessionPort {
  ...
  private async autoStartSessions(): Promise<void> {
    ...
    // duplicated work.
    const claimable = this.ownership?.claimableWhere() ?? [{}];
    // Include DISCONNECTED, READY, FAILED, and ACTION_REQUIRED sessions that have a phone number
    // (phone not null = was authenticated before). On restart, READY sessions are stale and need
    // re-initialization since the Chromium process is gone. FAILED sessions should also retry.
    const autoStartStatuses = [
    ...
    const sessions = await this.sessionRepository.find({
      where: claimable.flatMap(clause =>
        autoStartStatuses.map(status => ({ ...clause, phone: Not(IsNull()), status })),
      ),
    });
